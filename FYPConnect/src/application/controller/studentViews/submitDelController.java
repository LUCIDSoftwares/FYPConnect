...
public class submitDelController {
	...
	public void initialize() {
		...
		// get 4 lines from timeline_details.txt using filehandling
	...
	public void getTimeline() {
		// get 4 lines from timeline_details.txt using file io
		String[] timeline = new String[4];

		try {
			File file = new File("src/timeline_details.txt");
			if (file.exists()) {
				java.util.Scanner myReader = new java.util.Scanner(file);
				int i = 0;
				while (myReader.hasNextLine()) {
					timeline[i] = myReader.nextLine();
					i++;
				}
				myReader.close();
				System.out.println(timeline[0] + "  |  " + timeline[1] + "  |  " + timeline[2] + "  |  " + timeline[3]);
				if (timeline != null) {
					timelineText.setText("Timeline Details:    " + 
							timeline[0] + "    |    " + timeline[1] + "    |    " + timeline[2] + "    |    " + timeline[3]);
					timelineText.setEditable(false);
					timelineText.setWrapText(true);
					timelineText.setStyle("-fx-font-size: 14; -fx-font-style: italic;");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void downloadResource() {
		...
		// download the file
		String newFilePath = this.resourceHandler.downloadResource(String.valueOf(rs.getDeliverableId()),
				rs.getDocLink());
		if (newFilePath == null)
			this.promptTextArea.setText("Unable to Download File. Please Try Again Later");
		else
			this.promptTextArea.setText(
					"The Resource has been downloaded successfully." + "You can see the file at " + newFilePath);
